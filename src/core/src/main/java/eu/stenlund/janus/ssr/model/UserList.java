...
public class UserList {
     ...
     */
    private UserList(List<User> users, int total, int six, int max, String locale) {
     ...
     */
    public static Uni<UserList> createModel(SessionFactory sf, int start, int max, String locale)
    {
        return Uni.combine().all().unis(
            sf.withSession(s -> User.getList(s, start, max)),
            sf.withSession(s -> User.getCount(s)))
        .combinedWith((users, nof) -> new UserList(
                users,
                nof.intValue(),
                start,
                max,
