...
public class ProductVersionList {
    ...
    private ProductVersionList(List<ProductVersion> versions, int total, int six, int max, String locale) {
     ...
     */
    public static Uni<ProductVersionList> createModel(SessionFactory sf, int start, int max, String locale)
    {
        return Uni.combine().all().unis(
            sf.withSession(s -> ProductVersion.getList(s, start, max)),
            sf.withSession(s -> ProductVersion.getCount(s))).
        combinedWith((list, count)-> new ProductVersionList(list, count.intValue(), start, max, locale));
    }  
